...
class Drain3Parser():
    ...
    def _set_logger(self):
        ...
        DrainLogger.info('Construct DrainLogger success, current working directory: %s, logs will be written in %s' % (os.getcwd(), LOG_ROOT))
        return DrainLogger
    ...
    def _pre_process(self, line):
        headers, regex = self.generate_logformat_regex('<Logrecord> <Date> <Time> <Pid> <Level> <Component> \[<ADDR>\] <Content>')
...
if __name__ == '__main__':
    ...
    parser = Drain3Parser(config_file=os.path.join(PROJECT_ROOT, 'conf/HDFS.ini'), persistence_folder=os.path.join(PROJECT_ROOT, 'datasets/HDFS/persistences'))
