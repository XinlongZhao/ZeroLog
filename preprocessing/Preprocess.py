...
class Preprocessor:
    ...
    def _set_logger(self):
        ...
        PreprocessorLogger.info('Construct PreprocessorLogger success, current working directory: %s, logs will be written in %s' % (os.getcwd(), LOG_ROOT))
    ...
    def label_distribution(self, train, dev, test):
        ...
        if dev:
            ...
            self.logger.info('Dev: %d Normal, %d Anomalous instances.', dev_label_counter['Normal'], dev_label_counter['Anomalous'])
        test_label_counter = Counter([inst.label for inst in test])
        self.logger.info('Train: %d Normal, %d Anomalous instances.', train_label_counter['Normal'], train_label_counter['Anomalous'])
        self.logger.info('Test: %d Normal, %d Anomalous instances.', test_label_counter['Normal'], test_label_counter['Anomalous'])
    ...
    def process(self, dataset, parsing, template_encoding, cut_func):
        ...
        if dataset == 'HDFS':
            dataloader = HDFSLoader(in_file=os.path.join(PROJECT_ROOT, 'datasets/HDFS/HDFS.log'), semantic_repr_func=template_encoding)
            ...
            dataloader = BGLLoader(in_file=in_file, dataset_base=dataset_base, semantic_repr_func=template_encoding)
            parser_config = os.path.join(PROJECT_ROOT, 'conf/BGL.ini')
        elif dataset == 'OpenStack':
            dataloader = OSLoader(in_file=os.path.join(PROJECT_ROOT, 'datasets/OpenStack/openstack_normal1.log'), ab_in_file=os.path.join(PROJECT_ROOT, 'datasets/OpenStack/openstack_abnormal.log'), semantic_repr_func=template_encoding)
