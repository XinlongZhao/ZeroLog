...
class HDFSLoader(BasicDataLoader):
    def __init__(self, in_file=None, datasets_base=os.path.join(PROJECT_ROOT, 'datasets/HDFS'), semantic_repr_func=None):
    ...
    def _load_hdfs_labels(self):
        with open(os.path.join(PROJECT_ROOT, 'datasets/HDFS/anomaly_label.csv'), 'r', encoding='utf-8') as reader:
            for line in reader.readlines():
                # token = line.strip().split(',')
                # block = token[0]
                # label = self.id2label[token[1]]
                # self.block2label[block] = label

                block, label = line.strip().split(',')
                self.block2label[block] = label
