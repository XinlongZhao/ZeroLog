...
from preprocessing.datacutter.SimpleCutting import cut_by_all, cut_by_HDFS_eq_filter
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--lstm_hiddens', type=int, default=216, help="lstm_hiddens")
   ...
   # process OpenStack
    ...
    processor_OpenStack = Preprocessor()
    template_encoder_OpenStack = Template_TF_IDF_without_clean() if dataset == 'NC' else Simple_template_TF_IDF()
    train_OpenStack, dev_OpenStack, test_OpenStack = processor_OpenStack.process(dataset=dataset, parsing=parser, cut_func=cut_by_all, template_encoding=template_encoder_OpenStack.present)
    # Load Embeddings
    vocab_OpenStack = Vocab()
    vocab_OpenStack.load_from_dict(processor_OpenStack.embedding)
    ...
    vocab = Vocab()
    new_embedding = {}
    for key in processor_OpenStack.embedding.keys():
        new_embedding[key] = processor_OpenStack.embedding[key]
    for key in processor_HDFS.embedding.keys():
        new_embedding[key + 596] = processor_HDFS.embedding[key]
    print(new_embedding.keys())
    vocab.load_from_dict(new_embedding)

    save_dir = os.path.join(PROJECT_ROOT, 'outputs')
    output_model_dir = os.path.join(save_dir, 'models/ZeroLog/OpenStack_HDFS/' + dataset + '_' + parser + '/model')
    if not os.path.exists(output_model_dir):
        os.makedirs(output_model_dir)
    
    best_file_name = f"OSall_HDFSeq_{alpha:.2f}_{beta:.2f}_{gamma:.2f}_numlayer{num_layer}_lstm{lstm_hiddens}_dropout{dropout:.2f}_gc{gc_lr:.1e}_d{d_lr:.1e}_best.pt"
    last_file_name = f"OSall_HDFSeq_{alpha:.2f}_{beta:.2f}_{gamma:.2f}_numlayer{num_layer}_lstm{lstm_hiddens}_dropout{dropout:.2f}_gc{gc_lr:.1e}_d{d_lr:.1e}_last.pt"
    ...
    if mode == 'train':
        ...
        for epoch in range(epochs):
            ...
            batch_num_train_OpenStack = int(np.ceil(len(train_OpenStack) / float(batch_size)))
            batch_iter_train_OpenStack = 0

            batch_num_dev_HDFS = int(np.ceil(len(dev_HDFS) / float(batch_size)))
            batch_iter_dev_HDFS = 0

            batch_num_dev_OpenStack = int(np.ceil(len(dev_OpenStack) / float(batch_size)))
            batch_iter_dev_OpenStack = 0

            total_bn = max(batch_num_train_HDFS, batch_num_dev_HDFS, batch_num_train_OpenStack, batch_num_dev_OpenStack)

            meta_train_loader_HDFS = data_iter(train_HDFS, batch_size, True)
            meta_train_loader_OpenStack = data_iter(train_OpenStack, batch_size, True)

            meta_test_loader_HDFS = data_iter(dev_HDFS, batch_size, True)
            meta_test_loader_OpenStack = data_iter(dev_OpenStack, batch_size, True)
            ...
            for i in range(total_bn):
                ...
                meta_train_batch_OpenStack = meta_train_loader_OpenStack.__next__()
                tinst_tr_OpenStack = generate_tinsts_binary_label(meta_train_batch_OpenStack, vocab_OpenStack)
                tinst_tr_OpenStack.to_cuda(device)

                # SX_spt, TX_spt, SY_spt, gamma, beta
                loss = zerolog.forward(tinst_tr_OpenStack.inputs, tinst_tr_HDFS.inputs, tinst_tr_OpenStack.targets, gamma, beta)

                loss_value = loss.data.cpu().numpy()
                loss.backward(retain_graph=True)

                batch_iter_train_HDFS += 1
                batch_iter_train_OpenStack += 1
                ...
                meta_test_batch_OpenStack = meta_test_loader_OpenStack.__next__()
                tinst_test_OpenStack = generate_tinsts_binary_label(meta_test_batch_OpenStack, vocab_OpenStack)
                tinst_test_OpenStack.to_cuda(device)

                loss_te = alpha * zerolog.finetunning(tinst_test_OpenStack.inputs, tinst_test_HDFS.inputs,
                                                    tinst_test_OpenStack.targets, gamma, beta)
                loss_value_te = loss_te.data.cpu().numpy() / alpha
                loss_te.backward()

                batch_iter_dev_HDFS += 1
                batch_iter_dev_OpenStack += 1
                # aggregate
                ...
                if batch_iter_train_OpenStack == batch_num_train_OpenStack:
                    meta_train_loader_OpenStack = data_iter(train_OpenStack, batch_size, True)
                    batch_iter_train_OpenStack = 0
                ...
                if batch_iter_dev_OpenStack == batch_num_dev_OpenStack:
                    meta_test_loader_OpenStack = data_iter(dev_OpenStack, batch_size, True)
                    batch_iter_dev_OpenStack = 0

            if test_HDFS:
                zerolog.logger.info('Testing on test set.')
                _, _, f = zerolog.evaluate(test_HDFS, threshold, vocab_HDFS, processor_HDFS.id2tag)
                zerolog.evaluate_domin(test_HDFS, vocab_HDFS, source=1)
                if f > bestF:
        ...
        if os.path.exists(last_model_file):
            ...
            accuracy = zerolog.evaluate_domin(test_HDFS, vocab_HDFS, source=1)
            print("HDFS last_model_Accuracy:", accuracy)
            precision, recall, f = zerolog.evaluate(test_HDFS, threshold, vocab_HDFS, processor_HDFS.id2tag)
            print("HDFS last_model_precision:", precision)
            print("HDFS last_model_recall:", recall)
            print("HDFS last_model_f:", f)
        if os.path.exists(best_model_file):
            zerolog.logger.info('=== Best Model ===')
            zerolog.model.load_state_dict(torch.load(best_model_file))
            accuracy = zerolog.evaluate_domin(test_HDFS, vocab_HDFS, source=1)
            print("HDFS Best_model_Accuracy:", accuracy)
            precision, recall, f = zerolog.evaluate(test_HDFS, threshold, vocab_HDFS, processor_HDFS.id2tag)
            print("HDFS Best_model_precision:", precision)
            print("HDFS Best_model_recall:", recall)
            print("HDFS Best_model_f:", f)
