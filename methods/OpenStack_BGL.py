...
from preprocessing.datacutter.SimpleCutting import cut_by_all, cut_by_BGL_eq_filter
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--batch_size', type=int, default=128, help="batch_size")
    ...
    argparser.add_argument('--lstm_hiddens', type=int, default=100, help="lstm_hiddens")
    argparser.add_argument('--dropout', type=float, default=0.3, help="dropout")
    argparser.add_argument('--gc_lr', type=float, default=2e-3, help="gc_lr")
    argparser.add_argument('--d_lr', type=float, default=1e-4, help="d_lr")
    ...
    for key in processor_BGL.embedding.keys():
        new_embedding[key + 596] = processor_BGL.embedding[key]
    print(new_embedding.keys())
    vocab.load_from_dict(new_embedding)

    save_dir = os.path.join(PROJECT_ROOT, 'outputs')
    output_model_dir = os.path.join(save_dir, 'models/ZeroLog/OpenStack_BGL/' + dataset + '_' + parser + '/model')
    if not os.path.exists(output_model_dir):
        os.makedirs(output_model_dir)

    best_file_name = f"OSall_BGLeq_{alpha:.2f}_{beta:.2f}_{gamma:.2f}_numlayer{num_layer}_lstm{lstm_hiddens}_dropout{dropout:.2f}_gc{gc_lr:.1e}_d{d_lr:.1e}_best.pt"
    last_file_name = f"OSall_BGLeq_{alpha:.2f}_{beta:.2f}_{gamma:.2f}_numlayer{num_layer}_lstm{lstm_hiddens}_dropout{dropout:.2f}_gc{gc_lr:.1e}_d{d_lr:.1e}_last.pt"
    ...
    if mode == 'train':
        ...
        for epoch in range(epochs):
            ...
            total_bn = max(batch_num_train_BGL, batch_num_dev_BGL, batch_num_train_OpenStack, batch_num_dev_OpenStack)
            ...
            for i in range(total_bn):
                ...
                loss = zerolog.forward(tinst_tr_OpenStack.inputs, tinst_tr_BGL.inputs, tinst_tr_OpenStack.targets, gamma, beta)
                ...
                loss_te = alpha * zerolog.finetunning(tinst_test_OpenStack.inputs, tinst_test_BGL.inputs,
