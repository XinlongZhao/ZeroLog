...
from preprocessing.datacutter.SimpleCutting import cut_by_HDFS_eq_filter, cut_by_BGL_eq_filter
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--num_layer', type=int, default=3, help="num_layer")
    argparser.add_argument('--lstm_hiddens', type=int, default=297, help="lstm_hiddens")
    ...
    output_model_dir = os.path.join(save_dir, 'models/ZeroLog/BGL_HDFS/' + dataset + '_' + parser + '/model')
    if not os.path.exists(output_model_dir):
        os.makedirs(output_model_dir)
    
    best_file_name = f"BGLeq_HDFSeq_{alpha:.2f}_{beta:.2f}_{gamma:.2f}_numlayer{num_layer}_lstm{lstm_hiddens}_dropout{dropout:.2f}_gc{gc_lr:.1e}_d{d_lr:.1e}_best.pt"
    last_file_name = f"BGLeq_11HDFSeq_{alpha:.2f}_{beta:.2f}_{gamma:.2f}_numlayer{num_layer}_lstm{lstm_hiddens}_dropout{dropout:.2f}_gc{gc_lr:.1e}_d{d_lr:.1e}_last.pt"
    ...
    if mode == 'train':
        ...
        for epoch in range(epochs):
            ...
            for i in range(total_bn):
                ...
                loss = zerolog.forward(tinst_tr_BGL.inputs, tinst_tr_HDFS.inputs, tinst_tr_BGL.targets, gamma, beta)
                ...
                loss_te = alpha * zerolog.finetunning(tinst_test_BGL.inputs, tinst_test_HDFS.inputs,
                                                    tinst_test_BGL.targets, gamma, beta)
